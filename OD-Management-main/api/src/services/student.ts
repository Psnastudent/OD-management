...
export class StudentService {
  ...
  async createStudent(data: {
    ...
    return await prisma.$transaction(async (tx) => {
      // If a user with the email already exists, link to that user instead of creating
      const existingUser = await tx.user.findUnique({ where: { email } });

      if (existingUser) {
        // If there's already a student linked to that user, bail out with a clear error
        const existingStudent = await tx.student.findUnique({ where: { userId: existingUser.id } });
        if (existingStudent) {
          throw new Error('A student with this email already exists');
        }

        // Update the existing user with provided details (password, phone, name) and set role to STUDENT
        ...
        // Create student linked to the existing user
      ...
      // No existing user; create user first
