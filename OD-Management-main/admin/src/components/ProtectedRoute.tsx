...
/* -------------------------------------------------------
   ...
   - Shows your animated loading screen
------------------------------------------------------- */
export function ProtectedRoute() {
  const { user, isAuthenticated, isLoading, isAuthenticating } = useAuth();

  // STILL CHECKING authentication (loading)
...
/* -------------------------------------------------------
   PUBLIC ROUTE
   - Only NOT-logged-in users can access (login/signup)
...
export function PublicRoute() {
  ...
  // IF LOGGED IN → cannot see login/signup → redirect to dashboard
  if (isAuthenticated && user) {
